...
--
-- This revision changes the auto-sell parameter from a single
-- boolean (`p_auto_sell_sub_ar`) to a text[] of rarity codes
-- (`p_auto_sell_rarities`). NULL or empty array means no auto-sell.
-- The old boolean signature is dropped explicitly so both prod and
-- dev only carry the new text[] signature.
-- ============================================================

drop function if exists record_pack_pulls_batch(uuid, text, jsonb, boolean);
