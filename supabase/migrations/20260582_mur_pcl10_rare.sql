-- ============================================================
-- MUR 카드 감별 시 PCL10 확률 0.3% → 0.1%, PCL9 3.7% → 3.9%.
-- 다른 등급(C/U/R/RR/MA/AR/SR/SAR/UR) 은 직전 (20260579) 그대로:
--   PCL10 = 0.3%, PCL9 = 3.7%.
--
-- v_roll CASE 의 9/10 임계값을 rarity 에 따라 분기:
--   when v_rarity = 'MUR' → 99.9 (PCL10 0.1%)
--   else                  → 99.7 (PCL10 0.3%)
-- 다른 분기 (실패 70 / 6 / 7 / 8) 은 동일.
--
-- 두 함수 (단건 / 일괄) 모두 같이 갱신.
-- ============================================================
...
begin
  ...
  v_threshold_9 := case when p_rarity = 'MUR' then 99.9 else 99.7 end;
