-- ============================================================
-- Economy rebalance
--   · Showcase prices ~2x (basic 10k→20k · glass 30k→70k ·
--     premium 100k→250k · legendary 300k→700k)
--   · PCL bulk-sell prices halved (10→100k, 9→50k, 8→10k, 6·7→5k)
--   · Sabotage loot ratio 50% → 80% of the showcase's purchase price
-- ============================================================
...
language sql immutable as $$
  select case p_type
    ...
    when 'glass'     then 70000
    when 'premium'   then 250000
    when 'legendary' then 700000
    else null
  end
$$;
...
language sql immutable as $$
  select case
    when p_grade = 10 then 100000
    when p_grade = 9  then  50000
    when p_grade = 8  then  10000
    when p_grade in (6, 7) then 5000
    else 0
  end
$$;

-- Rebuild sabotage_card with loot @ 80% of the current showcase price.
