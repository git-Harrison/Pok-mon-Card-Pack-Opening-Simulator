-- ============================================================
-- 체육관 속성 매칭 강제 — 도전/방어 펫 모두 체육관 속성과 동일해야.
--
-- 사용자 요청: "속성 체육관인데 도전 하는 포켓몬도 같은 속성포켓몬만
-- 사용할 수 있어야".
--
-- 규칙:
--   체육관 속성 = 도전자 펫 3마리 모두 같은 속성 + 방어 덱 펫 3마리도
--   같은 속성. 어긋나면 RPC 거부.
--
-- 영향 RPC:
--   1) resolve_gym_battle — p_pet_types 배열 검증.
--   2) set_gym_defense_deck — 동일 검증.
--
-- 클라이언트도 picker 단계에서 필터링하지만, 서버가 마지막 권위.
-- ============================================================

-- 1) resolve_gym_battle — 펫 속성 체육관 속성과 일치 강제.
...
begin
  ...
  then
    ...
    for i in 1..3 loop
      ...
      v_enemy_states := v_enemy_states || jsonb_build_object(
        ...
        'type', v_def_pet.type, 'rarity', v_def_pet.rarity,
        'grade', v_def_pet.grade,
        'hp_max', v_def_pet.hp, 'hp', v_def_pet.hp,
        'atk', v_def_pet.atk, 'is_defender', true
      );
    end loop;
  else
  ...
  return json_build_object(
    ...
    'pets', v_pet_states, 'enemies', v_enemy_states,
    'turn_log', v_turn_log,
...
-- 2) set_gym_defense_deck — 방어덱 펫 속성도 체육관 속성 일치 강제.
...
begin
  ...
  for v_pet_id in select unnest(p_pet_grading_ids) loop
    if not exists (
      select 1 from psa_gradings g
       where g.id = v_pet_id and g.user_id = p_user_id
         and g.grade = 10 and g.id = any(coalesce(v_main_ids, '{}'::uuid[]))
    ) then
      return json_build_object('ok', false, 'error',
