-- ============================================================
-- Sabotage cost rebalance + 50% payback to victim.
--
-- Cost: was hard-coded per tier (30k / 50k / 100k / 200k).
--       Now derived as 10% of the showcase's purchase price.
--       basic     10,000  → 1,000p
--       glass     100,000 → 10,000p
--       premium   300,000 → 30,000p
--       legendary 1,000,000 → 100,000p
--
-- Payback: every sabotage attempt — success OR failure — credits
-- 50% of the cost to the showcase owner. Removes the "all loss,
-- no upside" defender experience and gives the owner a steady
-- trickle whenever someone targets them.
-- ============================================================
...
begin
  ...
  -- Owner gets 50% of the attempt cost regardless of outcome.
  v_payback := floor(v_cost * 0.5)::int;
  if v_payback > 0 then
    update users set points = points + v_payback
      where id = v_showcase.user_id;
  end if;
  ...
  if v_success then
    ...
    v_loot := floor(showcase_price(v_showcase.showcase_type) * 0.8)::int;
