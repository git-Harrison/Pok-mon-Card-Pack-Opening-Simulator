-- ============================================================
-- resolve_gym_battle — capture reward 150,000 → 15,000 (1/10 경제 정책).
--
-- 20260711 본문 그대로 + capture reward 상수만 변경 (line 327):
--   v_capture_reward := round(150000 * v_difficulty_mult)::int;
--   →
--   v_capture_reward := round(15000 * v_difficulty_mult)::int;
--
-- 그 외 모든 로직 (turn-order, dual-type eff, defender 처리, 보호기간,
-- 메달, 챌린지 status 갱신 등) 은 1바이트도 변경 없음.
--
-- 기존 유저 잔액 / gym_rewards 로그는 미터치 — 앞으로의 capture 트랜잭션만
-- 새 base 적용.
-- ============================================================
...
begin
  ...
  if v_winner = 'won' then
    ...
    -- ▶ 1/10 경제 정책 — capture base 150000 → 15000.
    v_capture_reward := round(15000 * v_difficulty_mult)::int;
...
-- 마이그레이션: 20260715_resolve_gym_battle_capture_div10.sql
