-- ============================================================
-- 체육관 풀 정책 — 보유 PCL10 + 속성 일치만 (rarity / 등록 무관).
--
-- 사용자 요구:
--   "공격덱 선택 시 펫 등록된 카드만 체크하지 말고, 보유한 카드 중
--    체육관과 같은 속성이면 다 사용할 수 있게."
--
-- 직전 정책 (20260733/20260739):
--   • MUR / UR PCL10: 보유 슬랩 모두 가능.
--   • 그 외 희귀도 PCL10: main_card_ids ∪ main_cards_by_type 등록된
--     슬랩만 가능.
--
-- 본 마이그레이션 (20260731 정책으로 회귀):
--   • resolve_gym_battle / set_gym_defense_deck — 본인 소유 + PCL10 +
--     속성 일치 (card_eligible_for_type) 만 통과. rarity / 등록 검사 제거.
--
-- 중복 정책 (20260739) 그대로 유지 — distinct grading_id 만 강제, 같은
-- card_id 슬랩 3장으로 덱 구성 가능.
-- set_pet_for_type 도 20260739 그대로 유지.
-- ============================================================
...
begin
  ...
  -- 출전 슬랩 검증 — 본인 소유 + PCL10 만. rarity / 등록 검사 제거.
...
begin
  ...
  -- 슬랩 검증 — 본인 소유 + PCL10 만. rarity / 등록 검사 제거.
  for v_pet_id in select unnest(p_pet_grading_ids) loop
    if not exists (
      select 1 from psa_gradings g
       where g.id = v_pet_id and g.user_id = p_user_id and g.grade = 10
...
-- 마이그레이션: 20260740_gym_pool_all_owned_pcl10.sql
