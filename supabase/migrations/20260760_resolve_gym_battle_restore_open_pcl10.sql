-- ============================================================
-- 버그 fix: resolve_gym_battle 가 "본인 펫(PCL10·등록 슬랩) 만 출전"
-- 으로 거부하는 회귀 수정 — 보유 PCL10 슬랩 누구나 출전 가능 정책 복구.
--
-- 원인:
--   20260755 (방어덱 OFF) 를 20260715 본문 기반으로 재작성하면서, 그
--   사이에 들어간 20260731 (체육관 풀 정책 완화 — 펫 등록 슬랩만 →
--   보유 PCL10 전부) 의 변경을 회귀시킴. 즉:
--   · v_main_ids = main_card_ids ∪ flatten_pet_ids_by_type 로딩 부활
--   · 검증 블록에 g.id = any(v_main_ids) 재추가
--   · distinct card_id (같은 카드 종류 1번만) 검사 삭제
--
-- 복구 내용 (20260755 NPC-only 정책 + 20260731 완화 머지):
--   1) v_main_ids / v_by_type_data 제거
--   2) 출전 슬랩 검증 → 본인 소유 + PCL10 만 (main_card_ids 검사 제거)
--   3) 에러 메시지 "본인 PCL10 슬랩만 출전할 수 있어요." 로 단순화
--   4) distinct card_id 검사 재도입 (같은 카드 종류는 한 덱에 1번)
--   5) 그 외 20260755 와 동일 — defender 경로 없음, 항상 gym_pokemon NPC
--
-- 멱등.
-- ============================================================
...
begin
  ...
  -- distinct grading_id + distinct card_id — 같은 슬랩 / 같은 카드 종류
  -- 모두 한 덱에 한 번만.
  ...
  -- 출전 슬랩 검증 — 본인 소유 + PCL10 만 (펫 등록 여부 무관, 20260731).
...
-- 마이그레이션: 20260760_resolve_gym_battle_restore_open_pcl10.sql
