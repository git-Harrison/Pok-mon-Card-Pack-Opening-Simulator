-- bulk_submit_psa_grading 500 에러 핫픽스
-- 20260540에서 도입한 인라인 cap-skip 로직이 일부 환경에서 500을 반환.
-- 안전한 형태로 재정의: assert_pcl_cap 으로 사전 차단하되, 한도 초과 시
-- raise 메시지를 P0001 errcode로 던져 클라이언트가 그대로 노출하도록 한다.
-- 부분 성공이 필요한 경우는 추후 별도 RPC로 분리.
