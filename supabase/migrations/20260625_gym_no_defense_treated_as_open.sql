-- ============================================================
-- 점령됐지만 방어 덱 미설정 = 비점령 으로 처리.
--
-- 사용자 정책: "점령자가 방어 덱 3종류를 안넣으면 점령이 된게
-- 아니라고 판단". 이전 (20260620/20260621) 은 'defender_deck_not_set'
-- 명시 오류로 도전 차단했지만 정책 변경 — NPC 경로로 도전 진행 가능,
-- 도전자 승리 시 새 owner 로 overwrite (이전 owner 가 자동 박탈).
--
-- 변경:
--   resolve_gym_battle 의 점령 분기에서:
--     · defense_pet_ids null/3 미만 → NPC 경로로 fall through.
--     · stale 슬랩 valid_count != 3 → ownership reset (defense null) +
--       동일 NPC 경로 (오류 X).
--   기존 enemy_count 검증 / pet stat NULL 가드는 유지.
--
-- (get_gyms_state.has_defense_deck 는 이미 valid 3 검사라 owner 가
--  방어덱 없으면 false 반환 → 클라가 NPC pokemon 표시. 추가 변경 X.)
-- ============================================================
...
begin
  ...
  -- 펫 등록 풀: legacy main_card_ids ∪ main_cards_by_type union.
  ...
  -- 적 빌드 — 점령 + valid 방어덱 일 때만 defenders, 그 외는 NPC.
  ...
  then
    ...
    if v_def_valid_count = 3 then
      ...
      -- stale → ownership reset (defense null) 후 NPC 로 fall through.
      update gym_ownerships
         set defense_pet_ids = null, defense_pet_types = null
       where gym_id = p_gym_id;
      -- v_owner_record 재로드 — 후속 challenge 처리에서 사용.
      select * into v_owner_record from gym_ownerships where gym_id = p_gym_id;
    end if;
  end if;
  ...
  if v_use_defenders then
    ...
    -- NPC 분기: 미점령 OR 점령자 방어덱 미설정/stale.
  ...
  if v_winner = 'won' then
    ...
    -- 이전 owner 가 valid defenders 로 싸웠을 때만 defense 슬랩 destroy.
    -- 방어덱 없이 NPC 로 진 owner 는 슬랩 손실 없음 (방어 안 하므로).
    if v_use_defenders
