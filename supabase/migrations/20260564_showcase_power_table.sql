-- ============================================================
-- 전시 슬랩 전투력 — rarity_power(r) * pcl_power(g) 곱셈 모델
-- 폐기, (rarity, grade) 쌍을 직접 lookup 하는 showcase_power(r, g)
-- 로 교체.
--
-- 새 표 (PCL 10 / PCL 9):
--   MUR  100 / 90
...
--   R     20 / 18
--   U     10 / 9
--   C      6 / 5     (PCL 9 = 5 — floor 5.4)
--
--   PCL 9 = floor(PCL 10 × 0.9). PCL 6/7/8 은 0 (변경 없음).
--
-- 이전 곱셈 모델은 AR / RR / R / U / C 가 0 이라 전시 슬랩이
-- MUR/UR/SAR/MA/SR 5종 외엔 점수 없었음. 새 표는 모든 등급 0+.
--
-- 영향 RPC: 곱셈 패턴이 박혀있던 3곳을 lookup 호출로 교체:
--   1) get_user_rankings  (center_power)
--   2) get_profile         (center_power)
--   3) get_user_activity   ('power' 탭, 전시 이벤트 라벨)
-- 다른 곳에서의 rarity_power × pcl_power 곱셈은 모두 폐기된
-- 정의(이전 마이그레이션) 안에 있으므로 건드리지 않음.
-- ============================================================

-- 1) (rarity, grade) → 점수 lookup
...
as $$
  select case
    ...
    when p_rarity = 'AR'  and p_grade = 10 then 60
    when p_rarity = 'AR'  and p_grade = 9  then 54
    when p_rarity = 'SR'  and p_grade = 10 then 50
    when p_rarity = 'SR'  and p_grade = 9  then 45
...
-- 2) get_user_rankings — center_power 의 전시 합산을 showcase_power 로
...
-- 3) get_profile — center_power 의 전시 합산을 showcase_power 로
...
-- 4) get_user_activity('power') — 전시 이벤트 점수도 showcase_power 로
  ...
  if p_tab = 'rank' then
    ...
    with showcases as (
      select
        ...
        showcase_power(g2.rarity, g2.grade) as points,
