-- ============================================================
-- hun 계정 시드 — MUR 제크로무 ex (sv11b-174) PCL10 슬랩 40장 카드지갑 추가.
--
-- 사용자 요구:
--   "hun 계정에 MUR 제크로무 카드지갑에 40장 시드로 넣어줘"
--
-- 정책:
...
--
-- 멱등:
--   · 현재 보유 PCL10 슬랩 수가 40장 미만이면 부족분만 채움.
--   · 이미 40장 이상 보유 시 skip.
--
...
declare
  ...
  v_target_count constant int := 40;
  v_existing int;
  v_to_insert int;
begin
  select id into v_user_id from users where user_id = 'hun';
  if not found then
    raise notice '[hun zekrom x40] user hun 미존재 — skip';
    return;
  end if;
  ...
  if not exists (
    ...
    raise notice '[hun zekrom x40] card_types 에 % (MUR) 없음 — skip', v_card_id;
    return;
  end if;
  ...
  if v_to_insert <= 0 then
    raise notice '[hun zekrom x40] 이미 % 장 보유 (목표 %) — skip',
  ...
  raise notice '[hun zekrom x40] 기존 % 장 → 목표 % 장 (% 장 추가, pcl_10_wins +%)',
